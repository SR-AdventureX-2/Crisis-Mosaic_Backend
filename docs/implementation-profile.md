...
- 图片保存在 `data/uploads` 的隔离目录，数据库只记录服务端生成的路径。
- 持久任务与 Outbox 保存在 SQLite；WebSocket 连接中心仅驻留当前进程。
...
- 人工决策必须覆盖当前完整证据，并在一个事务内更新冲突、证据处置、事实版本、地图、
  审计和 Outbox。
- 居民地图按用户覆盖要求返回真实精确坐标，但不返回设备 ID 或 Token。单次最多 500
  个点，bbox 还受配置面积上限约束，任一超限均返回 `MAP_VIEW_TOO_LARGE`。
...
| 存储 | 本地隔离文件目录 | 不等价于对象存储或预签名上传 |
| 身份 | 本地账号、JWT | 不提供 OIDC、MFA 或企业目录 |
| 密钥 | 本地 `.env` | 不等价于生产密钥管理系统 |
| 地图隐私 | 居民可见精确点，最多 500 | 明确覆盖原文中的模糊、聚合策略 |
